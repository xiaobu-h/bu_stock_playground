...
import pandas as pd 
import logging
from strategy.bl.sensitive_param import TAKE_PROFIT_PERCENT_LARGE, BAR,VOLUME_FOR_QUADRUPLE_WITCH_DAY,LOOKBACK_DAYS, VOLUME_MULTIPLIER, TAKE_PROFIT_PERCENT, STOP_LOSS_THRESHOLD, CROSS_DEEP_PCT, MIN_TOTAL_INCREASE_PERCENT
...
class BollingerVolumeBreakoutStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if  self.p.is_backtest and not self.ordered and  self.check_buy_signal():
            ...
            self.is_targeted = False
            self.buy_date = date
            self.inc = today_increase * self.data.open[0]
            if  self.data.close[-1] > self.data.open[-1] and (self.data.close[-1] - self.data.open[0]) /self.data.close[-1] > 0.04 :
        ...
        if  self.p.is_backtest and self.ordered and self.position:
           ...
           # if delta == 1 and self.data.open[0] > self.data.close[0] and  (self.data.open[0] - self.data.close[0]) >  self.inc * 0.8    :
             #       self.global_stats[date]["extra_counter"] += 1
               #     self.is_targeted = True
